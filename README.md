...
The Cloudflare Worker includes watchdog code that dispatches `pipeline.yml` if the latest successful Fast News run is stale. `site/wrangler.toml` registers one `*/30 * * * *` Cron Trigger. Configure `GITHUB_ACTIONS_TOKEN` as a Cloudflare secret before relying on it; `GITHUB_REPOSITORY` is set in Worker vars.
